...
cMMTurnsGameNode::cMMTurnsGameNode(cocos2d::Size size, std::vector<float> inPlayerSkills, std::vector<int> inGameLevel, std::vector<int> inSubjects, cTeacherCard* teacherCard, float inGoal, int inTurnCount, int days)
{
	...
	//cards
	{
		auto dActiveCards = userDefault->profile.FindMember("aActiveCards");
		for (int i = 0; i < dActiveCards->value.Size(); i++)
		{
			for (int j = 0; j < userDefault->cards.size(); j++)
			{
				if (dActiveCards->value[i].GetString() == userDefault->cards.at(j)->id)
				{
...
cMMTurnsOutroNode::cMMTurnsOutroNode(cocos2d::Size size, int turns, int turnCount, float currentScore, float goal, int dayC)
{
	...
	//bottom btns
	{
		...
		exitBtn->btnUp = [&](cocos2d::Touch* touch, cocos2d::Node* node)
		{
			...
			cUserDefault *userDefault = cUserDefault::getInstance();
			auto level = userDefault->profile.FindMember("aLevel");
			...
			level->value[0].SetInt(sh + 1);
			level->value[1].SetInt(cl + 1);
			level->value[2].SetInt(sem + 1);
			level->value[3].SetInt(day + 1);
...
{
 	cUserDefault *userDefault = cUserDefault::getInstance();
	auto skills = userDefault->profile.FindMember("aStats");
	for (unsigned int i = 0; i < 8; i++) playerSkills.push_back(skills->value[i].GetFloat());
...
std::string cMMTurns::generateMode(std::string modeName)
{
	...
	if (userDefault->generatedModes.HasMember(gameName.c_str()))
		return userDefault->generatedModes.FindMember(gameName.c_str())->value.GetString();

	///Dinamic skill wrong!!!!!!!!!!!!
	auto skills = userDefault->profile.FindMember("aStats");
	std::vector<float> playerSkills;
	float minPS = skills->value[0].GetFloat(), maxPS = skills->value[0].GetFloat();
	for (unsigned int i = 0; i < 8; i++)
	{
		float val = skills->value[i].GetFloat();
	...
	rapidjson::Value s(result.c_str(), result.size(), userDefault->generatedModes.GetAllocator());
	rapidjson::Value key(gameName.c_str(), gameName.size(), userDefault->generatedModes.GetAllocator());
	userDefault->generatedModes.AddMember(key, s, userDefault->generatedModes.GetAllocator());
