...
cSMCardOutro::cSMCardOutro(int turns, int turnCounts, int dayCost, std::vector<int> cardsRares)
{
	...
	//bottom btns
	{
		...
		exitBtn->btnUp = [&](cocos2d::Touch* touch, cocos2d::Node* node)
		{
			...
			cUserDefault *userDefault = cUserDefault::getInstance();
			auto students = userDefault->generatedCards.FindMember("studentCards");

			auto level = userDefault->profile.FindMember("aLevel");
			std::vector<std::string> vec = Split(studentCard->rData, '/');
			rapidjson::Value key(vec.at(0).c_str(), vec.at(0).size(), userDefault->generatedCards.GetAllocator());
			rapidjson::Value val(vec.at(1).c_str(), vec.at(1).size(), userDefault->generatedCards.GetAllocator());
			students->value.AddMember(key, val, userDefault->generatedCards.GetAllocator());
