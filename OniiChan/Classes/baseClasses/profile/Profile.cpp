...
cProfile::cProfile(cocos2d::Size sizeProfile)
{
	...
	std::string avatarFilePath = userDefault->profile.FindMember("sAvatarName")->value.GetString();
	auto nameData = userDefault->profile.FindMember("sName");
	...
	cocos2d::Sprite *sAvatar = cocos2d::Sprite::create(avatarFilePath);
	{
		...
		sAvatar->setName("ava");
		setNodeInRect(sAvatar, sizeProfile * 0.7f, 0);
		...
		sFloor->setPosition(0, sAvatar->getPosition().y - sizeProfile.height * 0.35f);
		...
		cocos2d::Label *profileName = cocos2d::Label::createWithTTF(*gTtfConfig, nameData->value.GetString(), cocos2d::TextHAlignment::CENTER);
	...
	{
		cocos2d::Size avaSize = sAvatar->getContentSize() *  sAvatar->getScale();
		auto average = userDefault->profile.FindMember("fAverageScore");
		auto best = userDefault->profile.FindMember("fBestScore");
		auto last = userDefault->profile.FindMember("fLastScore");
		{
			infoElevent *el = new infoElevent();
			el->node = new cBaseNode();

			cocos2d::Label *aScore = cocos2d::Label::createWithTTF(*gTtfConfigHd, std::to_string(average->value.GetFloat()).substr(0, 3), cocos2d::TextHAlignment::CENTER);
...

		{
			...
			cocos2d::Label *bScore = cocos2d::Label::createWithTTF(*gTtfConfigHd, std::to_string(best->value.GetFloat()).substr(0, 6), cocos2d::TextHAlignment::CENTER);
...

		{
			...
			cocos2d::Label *lScore = cocos2d::Label::createWithTTF(*gTtfConfigHd, std::to_string(last->value.GetFloat()).substr(0, 6), cocos2d::TextHAlignment::CENTER);
	...
	//Diag
	{
		auto stats = userDefault->profile.FindMember("aStats");
		cocos2d::Size avaSize = sAvatar->getContentSize() *  sAvatar->getScale();

		for (int i = 0; i < 8; i++)profileSkills.push_back(stats->value[i].GetFloat());
