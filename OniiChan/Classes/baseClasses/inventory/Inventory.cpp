...
cCardInventory::cCardInventory(cocos2d::Size invSize)
{
	...
	this->addChild(slotList);
	{
		...
		slotList->setPosition(0, size.height / 2.0f - slotHeight / 2.0f - indent);
	}
	...
	this->listener->onTouchEnded = [&](cocos2d::Touch* touch, cocos2d::Event* event)
	{
		...
		if (abs(dS.x) < wait)
		{
			...
			if (rect.containsPoint(p) && btnAdd->isVisible()) //addButon click
			{
				selectCard();				
			}
	...
	this->secondUpdate = [&](float dt)
	{
		...
		if(!isPress && listPos[1].get() != 0 && !listPos[1].isProgress()) listPos[1].set(0, 1.5f, true);
		...
		//for (auto it : elements)it->update(dt, indent);
	};
	updateSlots();
	updateList();
}
...
void cCardInventory::updateStates()
{
	...
	{
		if (it.state == -1)pActiveCards.push_back(&it);
		else if (it.state == 1)pSelectedCards.push_back(&it);
	}
}
